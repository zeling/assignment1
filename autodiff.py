...
class AddOp(Op):
    ...
    def compute(self, node, input_vals):
        """Given values of two input nodes, return result of element-wise ad
        dition."""
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        if not isinstance(node.op, PlaceholderOp):
            grad_components = dict(zip(node.inputs, node.op.gradient(node, node_to_output_grad[node])))
            for input in node.inputs:
                grads_list = node_to_output_grads_list.get(input, [])
                grads_list.append(grad_components[input])
